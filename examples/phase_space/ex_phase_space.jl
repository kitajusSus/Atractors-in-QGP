...
function generuj_wykresy_przestrzeni()
    ...
    kolory = [paleta[1], paleta[4], paleta[7], paleta[10]]

    fig1 = Figure(size = (850, 600))
    ax1_2d = Axis(fig1[1, 1], xlabel = L"x", ylabel = L"v", title = L"\text{Przestrzeń fazowa} \; 2D \; (c = 0)")
    ...
    for (i, u0) in enumerate(warunki_poczatkowe)
        ...
        lines!(ax1_2d, x_pts, v_pts, color = kolory[i], linewidth = 8)

        lines!(ax1_3d, x_pts, v_pts, sol.t, color =kolory[i] , linewidth = 7)
    end

    fig3 = Figure(size = (850, 600))
    ax2_2d = Axis(fig3[1, 1], xlabel = L"x", ylabel = L"v", title = L"\text{Przestrzeń fazowa} \; 2D \; (c = 0.5)")
    ...
    for (i, u0) in enumerate(warunki_poczatkowe)
        ...
        lines!(ax2_2d, x_pts, v_pts, color = kolory[i], linewidth = 8)

        lines!(ax2_3d, x_pts, v_pts, sol.t, color = kolory[i], colormap = paleta, linewidth = 7)
    end
